            ...
            <ul class="nav nav-tabs">
                ...
                @if($server->isInstalled())
                    ...
                    <li class="{{ $router->currentRouteNamed('admin.servers.view.build') ? 'active' : '' }}">
                        <a href="{{ route('admin.servers.view.build', $server->id) }}">ビルド構成</a>
                    </li>
                    <li class="{{ $router->currentRouteNamed('admin.servers.view.startup') ? 'active' : '' }}">
                        <a href="{{ route('admin.servers.view.startup', $server->id) }}">起動</a>
                    </li>
                ...
                <li class="{{ $router->currentRouteNamed('admin.servers.templates') ? 'active' : '' }}">
                    <a href="{{ route('admin.servers.templates', $server->id) }}">テンプレートから作成</a>
                </li>
            </ul>
        </div>
    </div>
